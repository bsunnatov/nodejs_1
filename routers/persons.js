...
personsRouter.post('/', function (req, res) {
    const { firstName, lastName, age, dayOfBirth, address } = req.body;

    const person_string = fs.readFileSync('person.json', 'utf-8');
    const persons = [...JSON.parse(person_string)];
    persons.push(new Person(firstName, lastName, age, dayOfBirth, new Address(address?.regionId, '', address?.addressLine1)));
